...
[ -d "$CHART_ROOT" ] || exit_with_error_message "'CHART_ROOT[$CHART_ROOT]' does not exist. Aborting." 1
...
current_tag="$(yq eval "${VERSION_PATH}" "${CHART_ROOT}/Chart.yaml")"
...
echo "Packaging chart under : ${CHART_ROOT}"
...
echo "Bumping tag from $(yq eval "${VERSION_PATH}" "${CHART_ROOT}/Chart.yaml") to ${target_tag}"
yq e "${VERSION_PATH} = \"${target_tag}\"" -i "${CHART_ROOT}/Chart.yaml"
